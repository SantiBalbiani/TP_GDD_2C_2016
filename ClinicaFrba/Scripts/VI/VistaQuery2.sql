...
SELECT  TOP (5) P.matricula, P.apellido, P.nombre, E.descripcion,E.idEspecialidad, Af.plan_idPlan, COUNT(Af.plan_idPlan) AS 'CantConsultas'
FROM   Select_Group.Profesional AS P INNER JOIN Select_Group.Profesional_Por_Especialidad AS PE ON PE.profesional_idProfesional = P.matricula INNER JOIN Select_Group.Especialidad AS E ON E.idEspecialidad = PE.especialidad_idEspecialidad INNER JOIN Select_Group.Agenda AS Ag ON Ag.profesional_IdProfesional = P.matricula INNER JOIN Select_Group.Turno AS T ON T.idAgenda = Ag.idAgenda INNER JOIN Select_Group.Afiliado AS Af ON Af.idAfiliado = T.afiliado_idAfiliado 
WHERE Af.plan_idPlan = 555555 AND E.idEspecialidad = 10001 AND T.fechaTurno BETWEEN '01/01/1999' AND '12/12/2017'
GROUP BY P.matricula, Af.plan_idPlan, E.descripcion, P.apellido, P.nombre, E.idEspecialidad
